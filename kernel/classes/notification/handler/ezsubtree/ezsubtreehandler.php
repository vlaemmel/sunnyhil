...
$out_s = $collection->attribute( 'data_text' );

foreach ($addressList as $email) {
	
	$db = eZDB::instance();
	$query = "select password_hash from ezuser where email = '$email'";
	$results = $db->arrayQuery($query);

	$mypwhash = $results[0]['password_hash'];
	
	$new_out = str_replace('mypwhash', $mypwhash, $out_s);
	
	$addressList_part = array($email);
	
	if ($collection->attribute( 'data_subject' ) != "DONTSENDME") $transport->send( $addressList_part, $collection->attribute( 'data_subject' ), $new_out, null,
                      $parameters );
