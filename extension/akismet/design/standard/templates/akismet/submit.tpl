...
<tr>
	...
	<th class="name">Partial Content</th>
...
<tr class="{$sequence}">
	...
	<td>
	{set $myat = ezini(concat($node.class_identifier|wash(), '_AkismetSettings'), 'BodyAttribute', 'akismet.ini')}
	{if $myat}
	{set-block scope=relative variable=desc}
	{attribute_view_gui attribute=$node.data_map[$myat]}
	{/set-block}
	{$desc|strip_tags|shorten(100)}
	{/if}
	</td>
