<div class='module'  class="topmenu-design">
    ...
    <div id="topmenu-firstlevel-position">
        ...
        {if $top_menu_items_count}
           {foreach $top_menu_items as $key => $item}
				{set $name_s = $item.name|wash}
				{if $item.class_identifier|eq('blog')}{set $name_s = concat($name_s, " Archive")}{/if}
                ...
                {if eq( $item.class_identifier, 'link')}
                    <li id="node_id_{$item.node_id}"{if $item_class} class="{$item_class|implode(" ")}"{/if}><div><a {if eq( $ui_context, 'browse' )}href={concat("content/browse/", $item.node_id)|ezroot}{else}href={$item.data_map.location.content|ezroot}{if and( is_set( $item.data_map.open_in_new_window ), $item.data_map.open_in_new_window.data_int )} target="_blank"{/if}{/if}{if $pagedata.is_edit} onclick="return false;"{/if} title="{$item.data_map.location.data_text|wash}" class="menu-item-link" rel={$item.url_alias|ezroot}><span>{if $item.data_map.location.data_text}{$item.data_map.location.data_text|wash()}{else}{$name_s|wash()}{/if}</span></a></div></li>
                {else}
                    <li id="node_id_{$item.node_id}"{if $item_class} class="{$item_class|implode(" ")}"{/if}><div><a href={if eq( $ui_context, 'browse' )}{concat("content/browse/", $item.node_id)|ezroot}{else}{$item.url_alias|ezroot}{/if}{if $pagedata.is_edit} onclick="return false;"{/if}><span>{$name_s|wash()}</span></a></div></li>
                {/if}
            {/foreach}
        {/if}
        </ul>
