...
{if $attribute.content.show_comments}
	...
	{def $other_coms = fetch( content, list, hash(class_filter_type, 'include', class_filter_array, array('comment'), parent_node_id, $attribute_node.node_id, sort_by, array('published', 1) ) )}
    ...
    {if $can_read}
        ...
        {* Comment item START *}
        {if sum($comments|count, $other_coms|count)|gt( 0 )}
            <div id="ezcom-comment-list" class="ezcom-view-list">
	
	            {foreach $other_coms as $comment}
	                {node_view_gui view='line' content_node=$comment}
					{set $total_count = $total_count|inc}
	            {/foreach}
                ...
                <div class="ezcom-view-all">
                  <p>
                      {if and($total_count|gt( count( $comments ) ), 1|eq(0))}
