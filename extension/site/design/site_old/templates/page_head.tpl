...
{def $site_title=$module_result.content_info.persistent_variable['site_title']}
{else}
{let name=Path
...
<meta name="{$key|wash|downcase}" content="{$item|wash}"/>

{/foreach}
{foreach $site.meta as $key => $item}
{if and($key|eq('keywords'), is_set( $module_result.content_info.persistent_variable[$key] ), $module_result.content_info.persistent_variable[$key]|eq(''))}{continue}{/if}
{if is_set( $module_result.content_info.persistent_variable[$key] )}
<meta name="{$key|wash|downcase}" content="{$module_result.content_info.persistent_variable[$key]|addslashes}"/>
